#ifndef JSON_LED_PATTERNS2_H
#define JSON_LED_PATTERNS2_H
...
// JSONパターンの基底クラス
class JsonLedPattern2 {
public:
    JsonLedPattern2() : m_name("JSON Pattern") {}
    virtual ~JsonLedPattern2() {}
...
// カスタムJSONパターンの実装
class CustomJsonPattern2 : public JsonLedPattern2 {
public:
    CustomJsonPattern2() : JsonLedPattern2() {
        m_name = "Custom Pattern";
    }
    
    void parseJson(const JsonObject& json) {
        JsonLedPattern2::parseJson(json);
...
// パターンファクトリークラス
class PatternFactory2 {
public:
    static PatternFactory2& getInstance() {
        static PatternFactory2 instance;
        return instance;
    }
    
    JsonLedPattern2* createPattern(const JsonObject& json) {
        String type = "custom";
    ...
    void registerPatternType(const String& type, std::function<JsonLedPattern2*(const JsonObject&)> creator) {
...
private:
    ...
    static JsonLedPattern2* createCustomPattern(const JsonObject& json) {
        CustomJsonPattern2* pattern = new CustomJsonPattern2();
        pattern->parseJson(json);
        return pattern;
    }
    
    std::map<String, std::function<JsonLedPattern2*(const JsonObject&)>> m_creators;
};

// JSONパターンマネージャークラス
class JsonPatternManager2 {
public:
    JsonPatternManager2() {}
    ~JsonPatternManager2() {
        // パターンの解放
    ...
    bool loadPatternsFromJson(const String& jsonString) {
        ...
        if (doc.is<JsonObject>()) {
            ...
            JsonLedPattern2* pattern = PatternFactory2::getInstance().createPattern(patternObj);
            if (pattern) {
    ...
    JsonLedPattern2* getPatternByIndex(int index) {
    ...
    JsonLedPattern2* getPatternByName(const String& name) {
...
private:
    std::vector<JsonLedPattern2*> m_patterns;
    std::map<String, int> m_patternNameMap;
};

#endif // JSON_LED_PATTERNS2_H
